              ...
              <div class="card-header">
                <div class="row">
                  ...
                  <div class="col-lg-2 col-sm-12">
                    <a href="{{ route('ledger-posting.store') }}" class="btn btn-sm btn-outline-primary">Ledger
                      Posting</a>
                  </div>
                </div>
              </div>
                      ...
                      <td>
                        @if (count($expense) > 0)
                          @if (!$openingBlance && !$manualOpeningBlance && !$others_income)
                            <strong style="border-right:1px solid #ddd">{{ $income - $total }}</strong>
                          @elseif (!$openingBlance && !$manualOpeningBlance && $others_income)
                            <strong style="border-right:1px solid #ddd">{{ $income + $others_income - $total }}</strong>
                          @elseif (!$openingBlance && $manualOpeningBlance)
                            @if ($manualOpeningBlance->flag == 1)
                              <strong
                        ...
                        @else
                          @if (!$openingBlance && !$manualOpeningBlance && !$others_income)
                            <strong style="border-right:1px solid #ddd">{{ $income }}</strong>
                          @elseif (!$openingBlance && $manualOpeningBlance)
                            @if ($manualOpeningBlance->flag == 1)
                              <strong
